    ...
    window.AppCalendar = App.extend({
        handleFullcalendar: function() {
            var _options, my_events = [{
                ...
                my_options = {
                    ...
                    windowResize: function(view) {
                        var width = $(window).outerWidth(),
                            options = $.extend({}, my_options);
                        options.events = view.calendar.getEventCache(), options.aspectRatio = 667 > width ? .5 : 1.35, $("#calendar").fullCalendar("destroy"), $("#calendar").fullCalendar(options)
                    },
                    eventClick: function(event) {
                        var color = event.backgroundColor ? event.backgroundColor : $.colors("blue", 600);
                        $("#editEname").val(event.title), event.start ? $("#editStarts").datepicker("update", event.start._d) : $("#editStarts").datepicker("update", ""), event.end ? $("#editEnds").datepicker("update", event.end._d) : $("#editEnds").datepicker("update", ""), $("#editColor [type=radio]").each(function() {
                            var $this = $(this),
                                value = $this.data("color").split("|"),
                                value = $.colors(value[0], value[1]);
                            value === color ? $this.prop("checked", !0) : $this.prop("checked", !1)
                        }), $("#editColor [value=" + event.backgroundColor + "]").prop("checked", !0), $("#editNewEvent").modal("show").one("hidden.bs.modal", function(e) {
                            event.title = $("#editEname").val();
                            var color = $("#editColor [type=radio]:checked").data("color").split("|");
                            color = $.colors(color[0], color[1]), event.backgroundColor = color, event.borderColor = color, event.start = new Date($("#editStarts").data("datepicker").getDate()), event.end = new Date($("#editEnds").data("datepicker").getDate()), $("#calendar").fullCalendar("updateEvent", event)
                        })
                    },
                ...
                my_options_mobile = $.extend({}, my_options);
            my_options_mobile.aspectRatio = .5, _options = $(window).outerWidth() < 667 ? my_options_mobile : my_options, $("#editNewEvent").modal(), $("#calendar").fullCalendar(_options)
        },
        ...
        handleAction: function() {
            $(".site-action").actionBtn().data("actionBtn")
        },
        handleEventList: function() {
            $("#addNewEventBtn").on("click", function() {
                ...
                $this.data("event", {
                    ...
                    revertDuration: 0,
                    helper: function() {
                        return '<a class="fc-day-grid-event fc-event fc-start fc-end" style="background-color:' + $.colors(color[0], color[1]) + ";border-color:" + $.colors(color[0], color[1]) + '"><div class="fc-content"><span class="fc-title">' + $this.data("title") + "</span></div></a>"
                    }
                })
            })
        },
        ...
        run: function(next) {
            $("#addNewCalendarForm").modal({
                ...
                show: !1
            }), this.handleEventList(), this.handleFullcalendar(), this.handleAction(), this.handleSelective(), this.handleListItem(), next()
        }
