		...
		if($this->session->userdata('name_user') and $this->session->userdata('username')){
			if($this->model_user_access->access_access() == 1){
				...
				if(!empty($tgl_awal)){
					...
					$data_real = $this->db->query("SELECT * FROM tb_log WHERE cast(log_time as date) BETWEEN '$for_tgl_awal' AND '$for_tgl_akhir' ")->result();
					
				}else{

					$data_real	= $this->db->query("SELECT * FROM tb_log")->result();
				}

				$data = array('contents' => 'Dashboard/Log/index',
						  'data' => $data_real
							);
		...
		if($this->session->userdata('name_user') and $this->session->userdata('username')){
			if($this->model_user_access->access_access() == 1){
				...
				$data = array('contents' => 'Dashboard/Log/index',
					...
					header("Content-Disposition: attachment; filename=Rekap_Log.xls");
					header("Pragma: no-cache"); 
					header("Expires: 0");

					$this->load->view('Dashboard/Log/excel', $data);
