<?php
class BML  extends CI_Controller{
	...
	function index(){
		if($this->session->userdata('name_user') and $this->session->userdata('username')){
			if($this->model_user_access->access_access() == 1){
				...
				$data = array('contents' => 'Dashboard/BML/index',
						  'data' => $data_real
							);
		...
		if($this->session->userdata('name_user') and $this->session->userdata('username')){
			if($this->model_user_access->access_access() == 1){
				...
				$data = array(
					...
					header("Content-Disposition: attachment; filename=Rekap_Barang_Masuk.xls");
					header("Pragma: no-cache"); 
					header("Expires: 0");

					$this->load->view('Dashboard/BML/excel', $data);
