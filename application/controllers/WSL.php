<?php
class WSL extends CI_Controller{
	...
	function index(){
		if($this->session->userdata('name_user') and $this->session->userdata('username')){
			if($this->model_user_access->access_access() == 1){
				...
				$data = array('contents' => 'Dashboard/WSL/index',
						  'data' => $data_real
							);
		...
		if($this->session->userdata('name_user') and $this->session->userdata('username')){
			if($this->model_user_access->access_access() == 1){
				...
				$data = array('contents' => 'Dashboard/WSL/index',
					...
					header("Content-Disposition: attachment; filename=Rekap_Waste_Scrap.xls");
					header("Pragma: no-cache"); 
					header("Expires: 0");

					$this->load->view('Dashboard/WSL/excel', $data);
