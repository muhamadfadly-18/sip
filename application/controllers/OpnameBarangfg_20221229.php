<?php
class OpnameBarangfg  extends CI_Controller
{
    ...
    function index()
    {
        if ($this->session->userdata('name_user') and $this->session->userdata('username')) {
            if ($this->model_user_access->access_access() == 1) {
                $resultData = $this->db->query('SELECT * FROM tb_opname_barang_fg')->result();

                $data = array(
                    'contents' => 'Dashboard/OpnameBarangfg/index',
    ...
    public function add_action()
    {
        if ($this->session->userdata('name_user') and $this->session->userdata('username')) {
            if ($this->model_user_access->access_add() == 1) {
                ...
                $this->model_global->input_data($data, 'tb_opname_barang_fg');

                $this->session->set_flashdata('succeed', 'Data pengguna baru telah di Tambah.');
                redirect('OpnameBarangfg');
            } else {
                $this->load->view('Layouts/error-404');
            }
        } else {
    ...
    public function Validasibarang()
    {
        ...
        if ($OpnameBarang == 0) {
            ...
            $cektabel = $this->db->query("SELECT stok FROM tb_opname_barang_fg where id_barang = '$OpnameBarang' ")->row();
